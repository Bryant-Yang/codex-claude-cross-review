...
All profiles default Claude Code to the script-level read-only tool set `Read,Grep,Glob` for evidence inspection. Use `--claude-tools none` when you want Claude Code to review only the supplied diff/context without tools. Use `--claude-tools default` only when you intentionally want Claude Code to use its default tool set; that depends on Claude Code's own permission configuration and is not a script-level read-only sandbox.
Claude Code turn budgets are profile-based: `fast=12`, `normal=20`, and `deep=28`. Use `--claude-max-turns <n>` when Claude needs more read-only inspection turns for documentation or architecture reviews.
...
```bash
claude -p --output-format json --max-turns 20 --tools "" < prompt.md
```
